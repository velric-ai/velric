    ...
    <nav className={`fixed top-0 left-0 w-full z-50 px-4 sm:px-6 py-4 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center md:flex-row md:justify-between gap-4">
        ...
        <div className="flex justify-center gap-8 text-white font-medium text-sm sm:text-base flex-wrap w-full md:w-auto">
          ...
          <Link href="/register" className="whitespace-nowrap hover:text-purple-400 transition-colors duration-300 relative group">
            Register
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-purple-400 transition-all duration-300 group-hover:w-full"></span>
          </Link>
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-black/90 backdrop-blur-xl border border-purple-400/30 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                <div className="p-4 border-b border-purple-400/20">
                  ...
                  {user.user_metadata?.phone && (
                    <p className="text-gray-400 text-xs mt-1">{user.user_metadata.phone}</p>
                  )}
                </div>
                <Link href="/profile" className="block px-4 py-3 text-white hover:bg-purple-400/10 transition-colors duration-300">
                  My Profile
                </Link>
                <Link href="/missions" className="block px-4 py-3 text-white hover:bg-purple-400/10 transition-colors duration-300">
                  My Missions
                </Link>
                <button 
