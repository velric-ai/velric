...
export async function getMissionById(id: string): Promise<MissionTemplate | null> {
  if (USE_DUMMY) {
...
// User Missions
export async function createUserMission(userId: string, missionId: string, status: UserMission['status'] = 'suggested'): Promise<UserMission> {
  if (USE_DUMMY) {
    await new Promise(resolve => setTimeout(resolve, 100));
...
export async function updateUserMissionStatus(userId: string, missionId: string, action: 'star' | 'start' | 'submit' | 'complete'): Promise<UserMission> {
  const statusMap = {
...
export async function addGeneratedMissions(missions: MissionTemplate[]): Promise<void> {
  if (USE_DUMMY) {
    await new Promise(resolve => setTimeout(resolve, 50));
